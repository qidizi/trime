...
public class KeyboardView extends View implements View.OnClickListener {
    ...
    private void onBufferDraw() {
        ...
        for (int i = 0; i < keyCount; i++) {
            ...
            String hint_top = key.getHint_top();
            String hint_right = key.getHint_right();
            String hint_bottom = key.getHint_bottom();
            String hint_left = key.getHint_left();
            ...
            if (!Function.isEmpty(label)) {
                ...
                canvas.drawText(
                        ...
                        paint);
                if (mShowHint) {
                    float lr_y = (key.getHeight() - padding.top - padding.bottom) / 2
                            + (paint.getTextSize() - paint.descent()) / 2
                            + top
                            + key.getKey_text_offset_y();

                    if (!Function.isEmpty(hint_top)) {
                    ...
                    if (!Function.isEmpty(hint_right)) {
                        mPaintSymbol.setShadowLayer(mShadowRadius, 0, 0, mShadowColor);
                        canvas.drawText(
                                hint_right,
                                left + key.getWidth() * 4/9,
                                lr_y,
                                mPaintSymbol);
                    }

                    if (!Function.isEmpty(hint_bottom)) {
                        mPaintSymbol.setShadowLayer(mShadowRadius, 0, 0, mShadowColor);
                        canvas.drawText(
                                hint_bottom,
                    ...
                    if (!Function.isEmpty(hint_left)) {
                        mPaintSymbol.setShadowLayer(mShadowRadius, 0, 0, mShadowColor);
                        canvas.drawText(
                                hint_left,
                                left - key.getWidth() * 2 / 5,
                                lr_y,
                                mPaintSymbol);
                    }
                }
