...
public class Keyboard {
     ...
     */
    static float SEARCH_DISTANCE = 1.4f;
    /**
     * 按鍵默認水平間距
     */
    ...
    private boolean is_landscape; // 是否横屏
    ...
    public Keyboard(Context context) {
        is_landscape = Configuration.ORIENTATION_LANDSCAPE == context.getResources().getConfiguration().orientation;
        ...
        mDefaultHeight = is_landscape ? config.getPixel("key_height_landscape") : config.getPixel("key_height");
    ...
    public Keyboard(Context context, String name) {
        ...
        int height = Config.getPixel(m, is_landscape ? "height_landscape" : "height", 0);
        ...
        for (Map<String, Object> mk : lm) {
            ...
            if (column == 0) {
                int heightK = Config.getPixel(mk, is_landscape ? "height_landscape" : "height", 0);
    ...
    void setmShiftKey(Key mShiftKey) {
        this.mShiftKey = mShiftKey;
    }

    List<Key> getmComposingKeys() {
    ...
    String getAsciiKeyboard() {
        return mAsciiKeyboard;
    }

    boolean isLabelUppercase() {
